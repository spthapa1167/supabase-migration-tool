...
generate_result_html() {
    ...
    query_database_counts() {
        ...
        if [ -n "$pooler_host" ]; then
            ...
            policies=$(PGPASSWORD="$password" psql -h "$pooler_host" -p 6543 -U postgres.${project_ref} -d postgres -t -A -c "SELECT COUNT(*) FROM pg_policies WHERE schemaname NOT IN ('pg_catalog', 'information_schema', 'pg_toast', 'auth', 'vault', 'storage', 'realtime', 'pgbouncer', 'graphql_public', 'supabase_functions', 'supabase_functions_api', 'pgsodium', 'supavisor');" 2>/dev/null | tr -d ' ' || echo "0")
