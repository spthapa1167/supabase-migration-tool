...
Environment (Docker — same as edge_functions_migration.sh):
  EDGE_DOCKER_NO_AUTO_START=true   Do not run open -a Docker / systemctl; fail if daemon is down
  ...
  SKIP_DOCKER_CHECK=true           Skip Docker checks (not recommended)
...
if [ -z "$MIGRATION_DIR" ]; then
    ...
    # Prefer latest edge_functions_migration_*; else full schema_migration_* (same edge_functions/ layout)
