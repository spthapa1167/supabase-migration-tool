...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    try {
      ...
      folderRaw = String(body?.folder ?? "inbox").trim() || "inbox";
      action = String(body?.action || "").trim();
      const uidRaw = body?.messageUid ?? body?.message_uid;
      messageUid = parseInt(String(uidRaw ?? ""), 10);
    } catch  {
      return new Response(JSON.stringify({
    ...
    if (!accountId || !Number.isFinite(messageUid) || messageUid < 1) {
      return new Response(JSON.stringify({
        error: "accountId and messageUid are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const folderNorm = folderRaw.toLowerCase();
    try {
      ...
      if (action === "mark_seen") {
        ...
        const draftsName = await resolveTargetMailboxName(client, sanitizeFolderMappingForDraftsSelect(folderMapping), "drafts");
        await client.moveMessages(String(messageUid), draftsName, true);
      }
      return new Response(JSON.stringify({
        success: true
      }), {
    ...
    console.error("crm-webmail-message-action:", err);
    const message = err instanceof Error ? err.message : "Message action failed";
    return new Response(JSON.stringify({
      error: message
    }), {
      status: 200,
